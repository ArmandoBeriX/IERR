...
const NavigationBar = () => {
          ...
          if (isEditing && existingField) {
            ...
            if (existingFieldIndex !== -1) {
              ...
              updatedFields[existingFieldIndex] = {
                ...updatedFields[existingFieldIndex],
                ...fieldData,
                // Mantener propiedades críticas - CORRECCIÓN: mantener el id original
                id: updatedFields[existingFieldIndex].id, // NO cambiar el id
                tableId: tableId
              };
              ...
              // CORRECCIÓN: Si no se encuentra, crear como nuevo campo
              console.log('Creando como nuevo campo ya que no se encontró el existente');
  ...
  // CORRECCIÓN: Función para guardar entidad
  const handleSaveEntity = useCallback((entityData) => {
    console.log('Guardando entidad:', entityData);
    
    setTablesData(prevTablesData => {
      const updatedTablesData = prevTablesData.map(table => {
        if (table.id === currentEditingStateRef.current.tableId) {
          console.log('Encontrando tabla para actualizar:', table.id);
          return {
            ...table,
  ...
  // CORRECCIÓN: Función para abrir modal de edición de entidad
  const openEntityModal = useCallback((tableId, entity = null) => {
    ...
    setModalConfig({
      ...
      onSave: handleSaveEntity,
      onDelete: () => {
        // Para entidades, manejamos la eliminación directamente desde EntityForm
      },
      onClose: () => {
        setModalConfig(prev => ({ ...prev, isOpen: false }));
        // Limpiar el ref al cerrar
      ...
      showDeleteButton: false, // EntityForm maneja su propio botón de eliminar
      showSaveButton: true,
        ...
        <div className="modal-backdrop" onClick={modalConfig.onClose}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              {/* BOTÓN DE ELIMINACIÓN - SOLO PARA EDICIÓN DE CAMPOS */}
