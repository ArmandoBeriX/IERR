      ...
      const updatedTablesData = prevTablesData.map(table => {
        if (table.id === tableId) {
          ...
          if (isEditing && existingField) {
            ...
            if (existingFieldIndex !== -1) {
              // Actualizar campo existente - CORRECCIÓN: INCLUIR storeData COMPLETO
              ...
              updatedFields[existingFieldIndex] = {
                ...
                // CORRECCIÓN CRÍTICA: GUARDAR storeData COMPLETO
              ...
              console.log('✅ storeData guardado:', updatedFields[existingFieldIndex].storeData);
              console.log('✅ possibleValues guardado:', updatedFields[existingFieldIndex].storeData?.possibleValues);
            } else {
              console.warn('Campo no encontrado para editar. Creando como nuevo campo...');
              // CORRECCIÓN: Si no se encuentra, crear como nuevo campo INCLUYENDO storeData COMPLETO
              ...
              console.log('✅ storeData guardado:', newField.storeData);
              console.log('✅ possibleValues guardado:', newField.storeData?.possibleValues);
              updatedTable.tableFields = [...currentFields, newField];
            }
          } else {
            // Crear nuevo campo - CORRECCIÓN: INCLUIR storeData COMPLETO
            const newFieldId = `field-${tableId}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
            const newField = {
              id: newFieldId,
              tableId: tableId,
      ...
      try {
        ...
        if (savedTable) {
          ...
          if (savedField) {
            ...
            console.log('✅ possibleValues guardado correctamente:', savedField.storeData?.possibleValues);
